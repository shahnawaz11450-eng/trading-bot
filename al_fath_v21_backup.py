...
class ExecutionSimulator:
    ...
    def __init__(self, account_size_usd: float = 50_000,
                 max_pos_pct: float = 0.02, latency_bars: int = 2,
                 base_taker_bps: float = 5.0):
...
class UserDashboard:
    ...
    def render(user: UserAccount, equity_curve: List[float]) -> dict:
        ...
        sharpe = (np.mean(ret) / (np.std(ret) + 1e-9)) * np.sqrt(525_600) if ret else 0.0
...
def run_v21(csv_path: Optional[str] = None,
    ...
    exec_sim = ExecutionSimulator(account_size_usd=50_000, max_pos_pct=0.02,
